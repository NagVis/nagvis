...
foreach($type_tab["$mytype"] as $propname)
{
	...
	elseif($propname_ok == "host_name")
	{
		if(is_readable($ConfigPath.'hosts.cfg')) {
			$fh = fopen($ConfigPath.'hosts.cfg', r);
			while (!feof($fh)) {
				$file .= fread($fh, 8192);
			}
			if(preg_match_all('/'.$propname_ok.'(.*)/', $file, $content)) {
				print "<td class=\"tdfield_linetype\"><select name=\"$propname\">";
				print "<option value=\"\"></option>";
				for($i=0;$i<count($content[1]);$i++) {
					print "<option value=\"".$content[1][$i]."\">".$content[1][$i]."</option>";
				}
				print "</select></td>\n";
			}
			else {
	...
	elseif($propname_ok == "service_description")
	{
		if(is_readable($ConfigPath.'services.cfg')) {
			$fh = fopen($ConfigPath.'services.cfg', r);
			while (!feof($fh)) {
				$file .= fread($fh, 8192);
			}
			if(preg_match_all('/'.$propname_ok.'(.*)/', $file, $content)) {
				sort($content[1]);
				$content_unique = array_unique($content[1]);
				print "<td class=\"tdfield_linetype\"><select name=\"$propname\">";
				print "<option value=\"\"></option>";
				foreach($content_unique as $elements) {
	...
	elseif($propname_ok == "hostgroup_name")
	{
		if(is_readable($ConfigPath.'hostgroups.cfg')) {
			$fh = fopen($ConfigPath.'hostgroups.cfg', r);
			while (!feof($fh)) {
				$file .= fread($fh, 8192);
			}
			if(preg_match_all('/'.$propname_ok.'(.*)/', $file, $content)) {
	...
	elseif($propname_ok == "servicegroup_name")
	{
		if(is_readable($ConfigPath.'servicegroups.cfg')) {
			$fh = fopen($ConfigPath.'servicegroups.cfg', r);
			while (!feof($fh)) {
				$file .= fread($fh, 8192);
			}
			if(preg_match_all('/'.$propname_ok.'(.*)/', $file, $content)) {
	...
	elseif($propname_ok == "map_name") {
		if (is_dir($cfgFolder)) {
			print "<td class=\"tdfield_linetype\"><select name=\"$propname\">";
			print "<option value=\"\"></option>";
			if ($dh1 = opendir($cfgFolder)) {
				while (($cfgFolder = readdir($dh1)) != false) {
					if( preg_match('/.cfg/', $cfgFolder)) {
						$cfgValue = explode('.cfg', $cfgFolder);
						print "<option value=\"".$cfgValue[0]."\">".$cfgValue[0]."</option>";
					}
				}
			}
			closedir($dh1);
