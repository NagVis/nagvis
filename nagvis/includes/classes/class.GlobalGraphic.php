...
class GlobalGraphic {
	...
	function init(&$image) {
		if (DEBUG) debug('Start method GlobalGraphic::init()');
		...
		if (DEBUG) debug('End method GlobalGraphic::init()');
	}
	
	function middle($x1,$x2) {
		if (DEBUG) debug('Start method GlobalGraphic::middle('.$x1.','.$x2.')');
		$ret = ($x1+($x2-$x1)/2);
		if (DEBUG) debug('End method GlobalGraphic::middle(): '.$ret);
		return $ret;
	}
	
	function drawArrow(&$im,$x1,$y1,$x2,$y2,$w,$solid,$color) {
		if (DEBUG) debug('Start method GlobalGraphic::drawArrow('.$x1.','.$y1.','.$x2.','.$y2.','.$w.','.$solid.','.$color.')');
		...
		if (DEBUG) debug('End method GlobalGraphic::drawArrow()');
	}
	
	function newX($a,$b,$x,$y) {
		if (DEBUG) debug('Start method GlobalGraphic::newX('.$a.','.$b.','.$x.','.$y.')');
		$ret = (cos(atan2($y,$x)+atan2($b,$a))*sqrt($x*$x+$y*$y));
		if (DEBUG) debug('End method GlobalGraphic::newX(): '.$ret);
		return $ret;
	}
	
	function newY($a,$b,$x,$y) {
		if (DEBUG) debug('Start method GlobalGraphic::newY('.$a.','.$b.','.$x.','.$y.')');
		$ret = (sin( atan2($y,$x) + atan2($b,$a) ) * sqrt( $x*$x + $y*$y ));
		if (DEBUG) debug('End method GlobalGraphic::newY(): '.$ret);
		return $ret;
	}
}
