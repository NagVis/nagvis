...
class GlobalBackendndomy {
	...
	function findstateHost($hostName,$recognizeServices,$onlyHardstates) {
		...
		WHERE 
			...
			AND (h.instance_id='.$this->dbInstanceId.' AND h.host_object_id=o.object_id) 
	...
	function getHostNamesWithNoParent() {
		...
		AND (ph1.instance_id='.$this->dbInstanceId.' AND ph1.parent_host_object_id IS null)');
	...
	function getDirectChildNamesByHostName($hostName) {
		...
		if(isset($hostName) && $hostName != '') {
			...
			AND (h1.instance_id='.$this->dbInstanceId.' AND h1.config_type=1 AND o1.object_id=h1.host_object_id)
			AND o1.object_id=ph1.parent_host_object_id
			AND (h2.instance_id='.$this->dbInstanceId.' AND h2.config_type=1 AND ph1.host_id=h2.host_id)
	...
	function getHostsByHostgroupName($hostgroupName) {
		...
		$QUERYHANDLE = $this->mysqlQuery('SELECT 
			...
			WHERE 
				...
				AND (hg.instance_id='.$this->dbInstanceId.' AND hg.hostgroup_object_id=o.object_id) 
	...
	function getServicesByServicegroupName($servicegroupName) {
		...
		$QUERYHANDLE = $this->mysqlQuery('SELECT 
			...
			WHERE 
				...
				AND (sg.instance_id='.$this->dbInstanceId.' AND sg.servicegroup_object_id=o.object_id) 
